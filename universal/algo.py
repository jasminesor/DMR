...
class Algo(object):
    ...
    def run(self, S, n_jobs=1, log_progress=True) -> AlgoResult:
        ...
        self.run_time = (self.end_time - self.start_time) / len(B)
        ...
        if isinstance(S, ListResult):
            ...
            return AlgoResult(S[0].X, B, self.run_time)
        else:
            return AlgoResult(self._convert_prices(S, "ratio"), B, self.run_time)
