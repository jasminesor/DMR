...
class AlgoResult(PickleMixin):
    ...
    @property
    def mean_excess_return(self):
        """Mean Excess Return"""
        x = self.r - self.benchmark_r
        return x.mean()

    @property
    def calmar(self, w=None):
        """Calmar Ratio is APY/MDD"""
        freq = self.freq() or _freq((self.r - 1).index)

        # adjust rf rate by frequency
        rf = self.rf_rate / freq

        # subtract risk-free rate
        r = tools._sub_rf(self.r - 1, rf)
        ...
        mu = mu * freq
        return np.divide(mu, self.max_drawdown)
    ...
    @property
    def ubah_r(self):
        b = np.array([0 if s == "CASH" else 1 for s in self.X.columns])
        b = b / b.sum()
        w = self.X.shift(1) * self.B
        # normalize
        w = w.div(w.sum(axis=1), axis=0)
        w.iloc[0] = 1.0 / self.X.shape[1]
        ubah_r = (self.X - 1) * w
        # 组合日收益
        ubah_r = ubah_r.sum(axis=1) + 1
        # stock went bankrupt
        ubah_r[ubah_r < 0] = 0.0
        # add risk-free asset
        ubah_r -= (w.sum(axis=1) - 1) * self.rf_rate / self.freq()
        return ubah_r
    ...
    def alpha_ttest(self, market='ucrp'):
        # alpha因子的单样本t检验,检验alpha因子异于0的稳健性
        y = (self.r).cumprod()
        y.name = "r"
        if market == 'ubah':
            bases = (self.ubah_r).cumprod().to_frame()
        else:
            bases = (self.benchmark_r).cumprod().to_frame()
        bases.columns = ["market"]
        alpha = []
        for x in range(1, len(y)):
            alpha.append(tools.capm(y[0:x + 1], bases[0:x + 1], rf=self.rf_rate)["alpha"])
            # alpha.append(tools.capm(y[0:x + 1], bases[0:x + 1], rf=1.000156)["alpha"])
        return scipy.stats.stats.ttest_1samp(alpha / np.std(alpha), 0)
